...
parser.add_argument('--local_path', type = str, default='/home/mjh319/saves/combine/')
parser.add_argument("--wandb", default=False, action="store_true")
parser.add_argument('--saves', type = str, default='/home/mjh319/saves/')
...
eval_loader = dict()
for i in dataset:
    eval_loader[i] = torch.utils.data.DataLoader(dataset[i], batch_size=512, shuffle=False, num_workers=4, pin_memory=True)
...
# models_ = args.select
# # "(8_26)(18:7:33)0826_emb"
# # "(8_26)(17:43:43)0826_emb"
# saves_ = "(8_26)(17:43:43)0826_emb"
# emb_model.load_state_dict(torch.load("/home/mjh319/saves/"+ saves_ +"/_emb_net_"+str(models_)+".ckpt"))


# # pca, psd 두개 있음
# # pca_path = "pca_transform"+"_"+str(models_)+"_"+".pkl"
# pca_path = "./pca_1/pca_transform_512"+"_"+str(models_)+"_"+".pkl"
...
# emb_model = ResNet(ResidualBlock, [2, 2, 2], num_classes=100).to(device)
# emb_model = emb_model.to(device)
emb_model = ResNet(ResidualBlock, [2, 2, 2], num_classes=100, size=112).to(device)
emb_model = emb_model.to(device)


models_ = args.select
saves_ = args.saves
# "(9_14)(17:8:18)0826_emb"
# 9_16_17:44:57
# "9_15_7:22:18" 19

# ./pca_1/9_17_9:34:54_228_.pkl saves 9_17_9:34:54

base_path = "/home/mjh319/saves/ood/" + "cifar10"+"_pretrained_112"
path = "9_15_7:23:24"
emb_model_path = "/home/mjh319/saves/" +path
emb_model.load_state_dict(torch.load(base_path+"/emb_net.ckpt"))

# emb_model.load_state_dict(torch.load("/home/mjh319/saves/"+ saves_ +"/_emb_net_"+str(models_)+".ckpt"))

    
# pca_path = "./pca_imagenet/"+saves_+"_"+str(models_)+"_"+".pkl"
# # pca_path = "./pca_imagenet/"+saves_+"_"+str(models_)+"_"+".pkl"

# with open(pca_path, 'rb') as file:
#     transform_seman = pickle.load(file)
    
pca_path = base_path+"/seman_pca.pkl"
with open(pca_path, 'rb') as file:
    transform_seman = pickle.load(file)
    
print("pca {} saves {}".format(pca_path, saves_))    
...
# /home/mjh319/saves/(9_22)(10:41:56)temp_project
...
for epoch in range(args.num_epochs):
    ...
    losses_ = train(epoch, dataset_loader, loader, emb_model, loss_semantic, optimizer, semantic_net, args.train_data_name, nu, c_list, R_list, args.max_grad_norm, device,transform_seman, False, assignment)
...
#         torch.save(emb_model.state_dict(), save_path_emb)
        ...
        iid_score = get_loss_vals(flowlosslist_semantic, eval_loader[args.train_data_name], emb_model, \
                                  semantic_net, device, transform_seman, obtion_='semantic')
        
        iid_score[~np.isfinite(iid_score)] = 300000
        for loader__ in dataset_loader:  
            if loader__ != args.train_data_name :
                t = get_loss_vals(flowlosslist_semantic, eval_loader[loader__], emb_model, 
                                  semantic_net, device, transform_seman, obtion_='semantic')
