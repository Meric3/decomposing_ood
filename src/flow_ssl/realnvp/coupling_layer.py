...
from flow_ssl.realnvp.utils import checkerboard_mask, checkerboard_mask_tabulr
...
class MaskCheckerboard_Tabular:
    def __init__(self, reverse_mask):
        self.type = MaskType.CHECKERBOARD
        self.reverse_mask = reverse_mask
...
#         self.b = torch.zeros((1, dim), dtype=torch.float).to(x.device)
        ...
        self.b = checkerboard_mask_tabulr(dim, self.reverse_mask, device=x.device)
